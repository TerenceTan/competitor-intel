...
- Open question (Phase 2 scope): **Followers-only scrape vs. full-posts scrape.** Confirmed during Phase 1 EC2 smoke (2026-05-04): operator intent is to track follower counts across social platforms, NOT post content. The Phase 1 actor `apify/facebook-posts-scraper` was scaffolding to prove the pipeline (succeeded — 50 items returned, social_snapshots row written, confidence='medium' because the post-scraper actor doesn't surface follower_count). For Phase 2 production, evaluate a page-info actor (e.g. `apify/facebook-pages-scraper`) — typically cheaper per call than post scrapers and returns follower_count directly. Either swap the actor in `apify_social.py` or add a second per-page call. Cost re-projection needed.
- **Phase 2 actor selection — IG + X smoke results (2026-05-05):** Both proven on free tier.
  - **IG**: `apify/instagram-profile-scraper` at $0.0026/profile. Smoke: @icmarketsglobal returned 46,822 followers in ~3s. Input shape `{"usernames": ["<handle>"]}`. 76M total runs, 4.61★. Pay-per-event model with `profile=$0.0026`.
  - **X**: `kaitoeasyapi/twitter-x-data-tweet-scraper-pay-per-result-cheapest` at $0.00025/tweet. Smoke: @vantagemkts (IC_Markets has no X presence) returned 19,922 followers via embedded `author.followers` field. Input shape `{"searchTerms": ["from:<handle>"], "maxItems": 1}`. **Important**: actor returns minimum 20 items per call (cost ~$0.005/call) AND charges for 15 "mock_tweet" placeholders when the handle lookup 404s — must detect `type: "mock_tweet"` in production code and treat as zero-result. 36M runs, 4.41★, LIMITED_PERMISSIONS.
  - **Rejected actors and why:** `scrape.badger/twitter-user-scraper` (obscure mode-based input, schema not exposed); `apidojo/tweet-scraper` (requires "full account access" — security concern).
  - **Phase 2 cost projection (8 markets × 7 competitors = 56 calls/platform/run, weekly cadence):**
    - IG: 56 × $0.0026 = $0.15/run → $0.60/mo
    - X: 56 × ~$0.005 = $0.28/run → $1.12/mo
    - FB (existing `apify/facebook-posts-scraper`): 56 × $0.31 = $17.36/run → $69.44/mo
    - **Total: ~$71/mo. Exceeds free tier ($5).** Apify Starter ($49/mo includes ~$49 platform credit) is enough headroom. **Phase 2 prerequisite: upgrade Apify to paid tier.**
  - **Phase 2 follow-up optimization:** FB at $0.31/run dominates the cost. Consider swapping `apify/facebook-posts-scraper` for a profile-info actor (`apify/facebook-pages-scraper` or similar) — could drop FB cost from $69/mo to ~$0.50/mo bringing total to <$2/mo and possibly fitting in free tier.
- **Apify cost ceiling — free tier reality (D-06 reconciliation, 2026-05-04):** D-06 set a $100/mo cap assuming a paid Apify tier. Confirmed at EC2 deploy: account is on the Apify free tier with a ~$5 monthly platform-enforced credit (no user-settable Console "Usage limits" knob on free tier). Measured smoke run cost = $0.31 for 50 posts (1 competitor). At weekly cadence in Phase 1 (1 competitor) → ~$1.24/mo, comfortably under $5. Per-call cap of $1.00 in `apify_social.py` is the runaway-scroll guard; in worst case (cap hits every run) it consumes the full $5 free tier. Dashboard `APIFY_MONTHLY_CAP_USD` constant adjusted from 100 → 5 so the data-health cost-pct indicator tells the truth. **Phase 2 will require upgrading the Apify account to a paid tier** for 8-market × multi-competitor fanout — flag in Phase 2 kickoff.
